...
- **CI Linux build** -- Tauri's AppImage bundler requires `xdg-open`. Added `xdg-utils` to system deps.
...
- **Builder clock skew fix** -- Podman/Docker VM clock drift after sleep/wake caused `apt-get update` failures. Added `Acquire::Check-Date=false` and `sync_container_clock()`.
- **Builder doctor** -- new container clock skew check.
