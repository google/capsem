...
def rows(manifest_path: Path, manifest_url: str) -> list[tuple[str, str, int, str]]:
    ...
    listed = subprocess.run(
        [
            sys.executable,
            str(ROOT / "scripts" / "list-release-manifest-assets.py"),
            "--manifest-path",
            str(manifest_path),
            "--manifest-url",
            manifest_url,
    ...
    for line in listed.splitlines():
        if not line.strip():
            continue
        version, arch, name, digest, size, url = line.split("\t")
        found.append((url, digest.removeprefix("blake3:"), int(size), f"{version}/{arch}/{name}"))
    return found
