    ...
    try:
        finished = subprocess.run(
            [
                ...
                "--branch", CI_BRANCH,
                "--limit", str(CI_RUNS),
                "--json", "conclusion,displayTitle,url,createdAt",
            ],
            capture_output=True,
            text=True,
...
#: Outcomes that are not a verdict about the code. A cancellation says nothing,
#: and a run still going has not said anything *yet* -- `gh` reports its
#: conclusion as empty.
#:
#: Both are skipped rather than counted or treated as a break. Treating them as
#: a break is not a small error: the newest run on a busy trunk is very often
#: still going, so a wall of red reported itself as "Green on main" the first
#: time this was pointed at a live repository.
NOT_A_VERDICT = {"cancelled", "skipped", "", None}
...
def still_running(runs: list[dict]) -> int:
    """How many of the newest runs have not concluded."""
    return sum(1 for run in runs if run.get("conclusion") in {"", None})
...
def ci_section() -> str:
    ...
    running = still_running(runs)
    pending = f" ({running} still running)" if running else ""

    streak = consecutive_failures(runs)
    if not streak:
        return f"## Trunk CI\n\nGreen on `{CI_BRANCH}`{pending}.\n"

    latest = streak[0]
    return (
        "## Trunk CI is RED -- stop the line\n\n"
        f"`{CI_BRANCH}` has failed **{len(streak)} consecutive** "
        f"`{CI_WORKFLOW}` runs{pending}.\n"
