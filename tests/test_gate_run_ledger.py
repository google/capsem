...
def test_one_run_in_two_directories_is_recorded_once(checkout: Path) -> None:
    ...
    assert [row.run_id for row in runledger.rows(config)] == ["20260101-000000-aaaaaa-candidate"]
...
def test_the_ledger_is_trimmed_to_its_bound(checkout: Path) -> None:
    ...
    ledger.write_text(
        "\n".join(
            _row(
                f"20250101-{index:06d}-old-candidate", steps={"build": (1.0, "ok")}
            ).model_dump_json()
...
def test_sync_repairs_an_invalid_row_from_its_surviving_run(checkout: Path) -> None:
    """A bad row must not permanently suppress its authoritative run log."""
    config = gate_config.load(checkout)
    run_id = "20260101-000000-aaaaaa-candidate"
    _record(checkout, _events(run_id))
    ledger = runledger.path(config)
    ledger.parent.mkdir(parents=True, exist_ok=True)
    ledger.write_text(
        f'{{"row_schema":"{config.runlog.ledger.row_schema}","run_id":"{run_id}"}}\n',
        encoding="utf-8",
    )

    assert runledger.sync(config, config.runlog) == 1
    assert [row.run_id for row in runledger.rows(config)] == [run_id]
    assert ledger.read_text(encoding="utf-8").count(run_id) == 1
...
def test_an_invalid_history_row_never_replaces_the_real_failure(
    checkout: Path, monkeypatch: pytest.MonkeyPatch
) -> None:
    """Malformed advisory history is cleanup failure, not the gate verdict."""
    config = gate_config.load(checkout)

    def refuse_history(*_args: object) -> None:
        raise ValueError("invalid ledger row")

    monkeypatch.setattr(runledger, "append", refuse_history)
    with (
        pytest.raises(RuntimeError, match="the real failure"),
        RunLog.open(config, "candidate", argv=("candidate",)) as log,
    ):
...
def test_failures_are_counted_across_commands() -> None:
    ...
    history = [
        ...
        *[_row(f"r{n}", command="candidate", steps={"contracts": (1.0, "failed")}) for n in (2, 1)],
    ]
    analysis = analyse(history, DIGEST)
...
def test_following_one_step_is_not_scoped_to_the_latest_command() -> None:
    ...
    history = [
        _row("r3", command="test-fast", identity="fast", steps={"lint": (1.0, "ok")}),
        *[_row(f"r{n}", command="candidate", steps={"contracts": (10.0, "ok")}) for n in (2, 1)],
    ]
