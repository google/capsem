...
The manifest at cache/target/assets/manifest.json (v2 format) records per-arch assets
with their blake3 hashes. These tests check that for the host arch:

  - manifest.json parses as expected v2 shape
  - every file listed exists on disk under cache/target/assets/<arch>/
  - each file's blake3 hash matches the manifest entry
  - no unlisted files live in cache/target/assets/<arch>/, allowing hash-tagged aliases
