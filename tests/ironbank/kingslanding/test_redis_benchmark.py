...
def test_redis_guest_and_published_transport_samples(redis, service, tmp_path):
    bench = str(BIN_DIR / "capsem-bench-rs")
    doctor = subprocess.run(
        [bench, "doctor", "--json"], capture_output=True, timeout=15, check=False
    )
    (tmp_path / "doctor.json").write_bytes(doctor.stdout)
    (tmp_path / "doctor.stderr").write_bytes(doctor.stderr)
    identity = {
    ...
    for concurrency, pipeline in ((1, 1), (32, 1), (32, 16)):
        requests = (
            10000
            if concurrency == 1
            else 100000 * pipeline // (2 if pipeline > 1 else 1)
        )
            ...
            for lane in ("guest_local", "host_published"):
                ...
                if lane == "guest_local":
                    invocation = (
                        ...
                        + shlex.join(
                            ["capsem-bench-rs", *args, "--address", "127.0.0.1:6379"]
                        )
                    )
                ...
                assert document["metrics"]["completed_requests"]["samples"] == [
                    requests
                ]
                (tmp_path / f"{label}.json").write_text(raw)
    ...
    collectors = tmp_path / "collectors"
    collectors.mkdir()
    (collectors / "vsock").write_text(json.dumps({"metrics": metrics}))
    store = tmp_path / "benchmarks.db"
    result = subprocess.run(
        [
            bench,
            "run",
    ...
    assert result.returncode == 0, result.stderr
    (tmp_path / "record.json").write_bytes(result.stdout)
    ...
    (tmp_path / "report.txt").write_bytes(report.stdout)
