...
def _recipe_commands(name: str) -> tuple[str, ...]:
...
def test_public_fast_test_reaches_the_fast_graph_before_release_contracts() -> None:
    ...
    fast = "uv run --project build_system --frozen capsem-gate test-fast"
    contracts = "just _test-release-contracts"

    assert fast in commands and contracts in commands, (
        GUARD_SCHEDULING_RATIONALE + f"\n_test-fast commands drifted: {commands}"
    )
    assert commands.index(fast) < commands.index(contracts), (
        GUARD_SCHEDULING_RATIONALE
        + "\njust fast-test waits on release contracts before running the Citadel"
    )
