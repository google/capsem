...
RUST_PAIR = re.compile(
    r"self\.(?P<left>\w+)\.is_some\(\)\s*!=\s*self\.(?P<right>\w+)\.is_some\(\)"
)
...
predated it. A new contract applied to old bytes, with nothing that could have
told anyone in advance which published artifacts would stop satisfying it.
