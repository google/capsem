...
def test_release_workflows_do_not_make_vm_devices_world_writable() -> None:
    ...
    bootstrap = (ROOT / "build_system/scripts/bootstrap/bootstrap-linux-host.sh").read_text(
        encoding="utf-8"
    )
    helper = (ROOT / "build_system/packaging/shared/install-vm-device-access").read_text(
        encoding="utf-8"
    )
