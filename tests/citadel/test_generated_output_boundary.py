"""Keep generated bytes out of source and converge repository output under cache/target/."""
...
RATIONALE = """\
Repository-generated bytes belong under ignored cache/target/ owners. Root assets,
...
or remain wired into CI after their producers move. See the cache/target/ contract in
...
def _ambient_asset_records(sources: dict[str, str]) -> tuple[str, ...]:
    records = [
        ...
        for line in text.splitlines()
        if not line.lstrip().startswith(("#", "//", "*", "<!--", "`"))
        if re.search(r"(?<!cache/target/)assets(?:/|\b)", line)
...
def _synthetic(**changes: object) -> Observed:
    values: dict[str, object] = {
        ...
        "tracked": (),
        "source_subject": (),
        "ignored_probes": frozenset({"cache/target/assets/.boundary-probe"}),
        "prefix_exports": (),
...
def _synthetic_policy() -> dict[str, Any]:
    ...
    return {
        "approved_output_probes": ["cache/target/assets/.boundary-probe"],
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            _synthetic(source_subject=("cache/target/assets/manifest.json",)),
...
def test_test_evidence_and_coverage_use_canonical_target_roots() -> None:
    ...
    for stale in (
        ...
        "cache/target/linux-rust-coverage",
        "cache/target/nextest/ci/junit.xml",
    ):
