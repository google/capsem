<script lang="ts">
  ...
  const isSessionView = $derived(sidebarStore.activeView === 'sessions');
</script>
...
<footer class="flex flex-shrink-0 items-center justify-between border-t border-base-300 bg-base-200 px-3 py-1 text-xs text-base-content/60">
  ...
  <button
    class="flex items-center gap-1 rounded px-1.5 py-0.5 transition-colors {isSessionView
      ? 'bg-primary/15 text-primary'
      : 'hover:bg-base-300 hover:text-base-content'}"
    onclick={() => sidebarStore.setView(isSessionView ? 'terminal' : 'sessions')}
    title="Session stats"
  >
    <SessionsIcon />
    <span>Session</span>
