...
embed_pkg_script() {
    ...
    local combined="${maintainer_script}.with-${helper}"
    {
        ...
        sed -n '2,$p' "$SCRIPT_DIR/../shared/$helper"
...
for bin in capsem capsem-service capsem-process capsem-tui capsem-mcp capsem-port-router capsem-mcp-aggregator capsem-mcp-builtin capsem-gateway capsem-tray capsem-admin capsem-mock-server capsem-bench-rs; do
    src="$BIN_DIR/$bin"
    if [ -f "$src" ]; then
