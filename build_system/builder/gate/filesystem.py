...
def merge_tree(source: Path, target: Path) -> None:
    ...
    A source symlink passed as the tree root is refused: there is no containing
    tree in which to preserve that link, so following it would violate the
    boundary this helper exists to enforce.
    """
    if source.is_symlink():
        raise GateError(f"source tree is a symlink, refusing to follow it: {source}")
