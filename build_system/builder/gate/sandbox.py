...
from .sandboxprofile import profile
...
def mode(command_default: SandboxMode, requested: SandboxMode | None) -> SandboxMode:
...
def linux_wrap(config: GateConfig, argv: tuple[str, ...]) -> tuple[str, ...]:
    ...
    if shutil.which(settings.linux_command) is None:
        raise GateError(f"Linux gate sandbox requires {settings.linux_command}; run bootstrap.sh")
...
def applied(
    ...
    *,
    ...
    if not host.on_macos():
        raise GateError(f"the gate sandbox does not support host system {host.system()!r}")
    written = written_to(config, config.path(config.runlog.root), report=chosen == REPORT)
