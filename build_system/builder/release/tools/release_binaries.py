...
def _capture(runner: RunCommand, *argv: str) -> str:
    return runner.run(argv, capture=True).stdout.strip()


def _validate_start(runner: RunCommand, channel: str, source_commit: str) -> None:
...
def precheck_release_binaries(channel: str, source_commit: str, runner: RunCommand) -> None:
...
def release_binaries(channel: str, source_commit: str, runner: RunCommand) -> tuple[str, str]:
    _validate_start(runner, channel, source_commit)
    version = _project_version()
    _validate_version_cohort(version)
    tag = f"v{version}"
