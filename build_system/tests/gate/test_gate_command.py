...
def test_a_reexec_becomes_the_same_command_it_replaced(monkeypatch, tmp_path) -> None:
    ...
    It returned `caffeinate ... just test-full`, which is a different thing that
    ...
    marker = gate_config.load(Path(__file__).resolve().parents[3]).candidate.keep_awake_marker
