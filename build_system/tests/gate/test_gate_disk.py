...
def test_reclaiming_removes_the_declared_trees(tmp_path: Path) -> None:
    ...
    built = _occupy(config, "cache/target/tests/home")
...
def test_reclaiming_reports_what_each_tree_gave_back(tmp_path: Path) -> None:
    ...
    config = _checkout(tmp_path)
    _occupy(config, "cache/target/tests/home", size=4096)
    _occupy(config, "cache/target/build/image-workspace", size=1024)

    recovered = reclaim(config)

    assert recovered.trees["cache/target/tests/home"] > recovered.trees["cache/target/build/image-workspace"]
...
def test_a_tree_the_run_still_needs_is_kept(tmp_path: Path) -> None:
    ...
    _occupy(config, "cache/target/tests/home")

    reclaim(config, keep=("cache/target/gate-runs",))

    assert config.path("cache/target/gate-runs").exists()
    assert not config.path("cache/target/tests/home").exists()
...
def test_a_symlink_inside_a_reclaimed_tree_is_unlinked_not_followed(
    ...
    Somebody's `cache/target/tests/home/assets` pointing at a real asset tree, or a
    ...
    home = config.path("cache/target/tests/home")
...
def test_a_reclaimable_root_that_is_itself_a_symlink_is_only_unlinked(
    ...
    """Someone points `cache/target/tests/home` at a scratch volume; reclaiming it
    ...
    link = config.path("cache/target/tests/home")
...
def test_footprint_measures_each_tree_separately(tmp_path: Path) -> None:
    ...
    config = _checkout(tmp_path)
    _occupy(config, "cache/target/tests/home", size=2048)

    measured = footprint(config)

    assert measured["cache/target/tests/home"] >= 2048
    assert "cache/target/build/image-workspace" not in measured, "absent trees are not zero rows"
