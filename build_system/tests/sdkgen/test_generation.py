...
import pytest
from capsem_builder.sdkgen.generation import python_sources, synchronize
...
def test_generation_is_deterministic_and_check_does_not_write(tmp_path: Path) -> None:
    sources = python_sources(SPEC)
    assert sources == python_sources(SPEC)
...
def test_every_generated_source_change_is_detected_and_repaired(tmp_path: Path, mutation: str) -> None:
    sources = {"models": {"item.py": "value = 1\n"}, "_operations": {"call.py": "value = 2\n"}}
    synchronize(tmp_path, sources, check=False)
    handwritten = tmp_path / "client.py"
    handwritten.write_text("owned by the SDK author")
    changed = tmp_path / "models" / "item.py"
    if mutation == "edit":
        changed.write_text("broken")
    elif mutation == "delete":
        changed.unlink()
    else:
        changed = tmp_path / "_operations" / "stale.py"
