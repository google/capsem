        ...
        async fn $name() {
            for full in [false, true] {
                ...
                for (status, invalid) in [(case.success_status, None), (403, None), (case.success_status, Some(b"not-json".as_slice())), (case.success_status, Some(b"{}".as_slice()))] {
                    ...
                    if status != case.success_status {
                    ...
                    } else if invalid == Some(b"{}".as_slice()) && case.open_json {
                        assert_eq!(serde_json::to_value(result.unwrap()).unwrap(), serde_json::json!({}));
                        ...
                        else if let Some(expected) = case.response.as_object() {
                            for (key, value) in expected {
                                assert_eq!(&actual[key], value, "response field {key}");
                            }
                        } else { assert_eq!(actual, case.response); }
                    }
...
operation!(create_network, "createNetwork", CreateNetworkParams);
operation!(list_networks, "listNetworks");
operation!(get_network, "getNetwork", GetNetworkParams);
operation!(delete_network, "deleteNetwork", DeleteNetworkParams);
operation!(attach_network_member, "attachNetworkMember", AttachNetworkMemberParams);
operation!(detach_network_member, "detachNetworkMember", DetachNetworkMemberParams);
