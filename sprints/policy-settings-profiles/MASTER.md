...
| 18 | [S07b - Capsem Admin Tooling And Profile-Derived Images](S07b-capsem-admin-tooling.md) | In Progress | Profile-admin validation/schema, `profile init`, `settings init`, typed section editability gates, `capsem-admin image plan`, profile-derived `image build-workspace`, public `image build` routing, local asset `image verify`, `manifest generate`, fast/download `manifest check`, minisign manifest signing/verification, profile/asset signature verification, developer bootstrap proof, and OS package layout proof for the `capsem-admin` wrapper/Python payload have landed. Remaining: release recipe cutover to profile-derived builds, package/tool/SBOM/in-guest verification, and typed policy/detection pack validation once S08a finalizes the rule formats. |
