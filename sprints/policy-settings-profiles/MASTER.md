...
| 14 | [S07a - Profile Manifest, Packages, And Assets](S07a-profile-manifest-assets.md) | In Progress | Typed manifest v3 profile catalog/status parser, typed profile package/tool contracts, per-arch VM asset declarations, and the Draft 2020-12 profile schema artifact with Rust golden fixture tests have landed; remaining scope adds TOML-to-schema/Pydantic admin validation, profile payload install, first-use download, retention, and VM profile/revision/asset pins. |
