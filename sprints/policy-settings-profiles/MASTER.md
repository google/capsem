...
| 18 | [S07b - Capsem Admin Tooling And Profile-Derived Images](S07b-capsem-admin-tooling.md) | In Progress | Profile-admin validation/schema, `profile init`, `settings init`, typed section editability gates, `capsem-admin image plan`, local asset `image verify`, `manifest generate`, fast/download `manifest check`, minisign manifest signing/verification, profile/asset signature verification, and developer bootstrap `capsem-admin` proof have landed. Remaining: profile-derived image build, package/tool/SBOM/in-guest verification, release packaged-layout proof, and typed policy/detection pack validation once S08a finalizes the rule formats. |
