...
| 18 | [S07b - Capsem Admin Tooling And Profile-Derived Images](S07b-capsem-admin-tooling.md) | In Progress | Profile-admin validation/schema, required Profile V2 `ui`, `profile init`, guest-config-derived `profile init-builtins` generated `everyday-work`/`coding` base profiles, `settings init`, typed section editability gates, `capsem-admin image plan`, profile-derived `image build-workspace`, public `image build` routing, profile-required local/release asset build recipes, rootfs-generated package/tool inventory, local asset plus per-arch inventory-backed `image verify`, typed guest SPDX SBOM generation, typed doctor-bundle probe ingestion, profile-backed release-image boot gate, `manifest generate`, fast/download `manifest check`, minisign manifest signing/verification, profile/asset signature verification, developer bootstrap proof, and OS package layout proof for the `capsem-admin` wrapper/Python payload have landed. Remaining: typed policy/detection pack validation once S08a finalizes the rule formats. |
...
| 20 | [S08a - Rule Abstraction And Detection Architecture](S08a-rule-abstraction-detection-architecture.md) | In Progress | First decision slice landed: policy and detection are separate profile-owned rule families; enforcement uses real CEL via the Rust `cel` crate family; Sigma is a detection authoring/import format, not a blocking language; detection packs compile to normalized Capsem detection IR and runtime findings attach to resolved security events before telemetry/audit/logging/timeline sinks. Remaining: finish schema details, admin validation requirements, and downstream sprint rewrites. |
