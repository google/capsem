...
| 14 | [S07a - Profile Manifest, Packages, And Assets](S07a-profile-manifest-assets.md) | In Progress | Typed manifest v3 profile catalog/status parser, typed profile package/tool contracts, per-arch VM asset declarations, Draft 2020-12 schema + Rust fixtures, and Python Pydantic v2 profile/manifest models have landed; remaining scope adds Rust TOML-to-schema validation, profile payload install, first-use download, retention, and VM profile/revision/asset pins. |
