...
| 22 | [S08c - Rule Corpus, Backtest, And Admin Parity](S08c-rule-corpus-admin-parity.md) | Not Started | Build the shared policy-context/enforcement/detection/event corpus, offline `capsem-admin` backtest parity, Rust runtime parity, negative `event.*` fixtures, and real-session fixture generation after S08b's journal stabilizes. |
| 23 | [S08d - Security Engine Performance Benchmarks](S08d-engine-performance-benchmarks.md) | Not Started | Extend benchmarking after S08b/S08c stabilize canonical policy roots and real corpora, proving VM-originated enforcement allow/block/ask latency, detection matching speed, rule-count scaling, backtest/hunt scan rates, and artifact-backed marketing performance claims at the exit of S08. |
