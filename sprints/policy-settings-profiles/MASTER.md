...
| 18 | [S07b - Capsem Admin Tooling And Profile-Derived Images](S07b-capsem-admin-tooling.md) | In Progress | Profile-admin validation/schema and `profile init` landed, plus Profile V2 section-level `editable` gates enforced by service mutations, immutable through profile `PUT`, and preserved by forks. Remaining: profile-derived image plan/build/verify, manifest generate/check/sign, bootstrap/release install proof, and typed policy/detection pack validation once S08a finalizes the rule formats. |
