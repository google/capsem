...
| 24 | [S09 - CLI Integration](S09-cli-integration.md) | Done For Bedrock | Usable CLI surface for the bedrock contract: profile/catalog/revision, profile-backed VM create, enforcement/detection validate/compile/backtest/live registry/stats/hunt, logs/status/debug, and disabled confirm visibility. Further naming/output polish is post-bedrock unless S18 finds a release-blocking gap. |
...
| 26 | [S11 - Status, Debug, Provenance](S11-status-debug-provenance.md) | Done For Bedrock | Status/debug/logs explain the shipped bedrock truth: active settings, profiles, derived rules, MCP, skills, profile catalog state, package contracts, asset readiness, VM pins, enforcement/detection matches, engine provenance, and honest live-health fields without false S12 claims. Full live metrics polish remains S12. |
...
| 36 | [S18 - Full Verification And Release Gate](S18-full-verification-release-gate.md) | Active Release Gate | Backend/CLI/UI/E2E/install proof for the core Profile V2 bedrock release path. Current decision: run the broader `just smoke` / release packaging gate or explicitly accept the narrower replay matrix recorded in `tracker.md`. |
