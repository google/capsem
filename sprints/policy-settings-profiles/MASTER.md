...
| 14 | [S07a - Profile Manifest, Packages, And Assets](S07a-profile-manifest-assets.md) | In Progress | Typed manifest v3 profile catalog/status parser landed; typed profile package/tool contracts and per-arch VM asset declarations now parse/validate and flow into VM-effective settings; remaining scope adds schema/admin validation, profile payload install, first-use download, retention, and VM profile/revision/asset pins. |
