...
| 18 | [S07b - Capsem Admin Tooling And Profile-Derived Images](S07b-capsem-admin-tooling.md) | In Progress | Profile-admin validation/schema, `profile init`, `settings init`, typed section editability gates, `capsem-admin image plan`, local asset `image verify`, `manifest generate`, fast `manifest check`, and full manifest byte/hash download checks have landed. Remaining: profile-derived image build, package/tool/SBOM/in-guest verification, cryptographic signature verification/signing, bootstrap/release install proof, and typed policy/detection pack validation once S08a finalizes the rule formats. |
