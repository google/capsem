...
Land the `v1.1.xxx` release only after T11 has produced a locally installed,
...
- [P0] The target release line is `1.1.xxx`; T12 must fail if tag, changelog,
...
- [ ] Confirm T9 recorded the exact `1.1.xxx` version and all version files
  match it.
...
- [ ] Confirm tag `v1.1.xxx` does not already exist locally or remotely.
...
- [ ] Otherwise, create the release commit and immutable tag manually, push the
  branch and `v1.1.xxx` tag, then run `just release v1.1.xxx` to wait for CI.
...
- [ ] Record final version, release URL, CI run URL, package asset names,
  ...
  proof in `tracker.md`.
- [ ] Confirm GitHub marks `v1.1.xxx` as the latest release if intended.
...
|---|---|
| Version | tag, changelog, binary metadata, release page, and release assets all say the exact `1.1.xxx`. |
...
- [ ] `gh release view v1.1.xxx`
- [ ] `gh release download v1.1.xxx --pattern manifest.json -D /tmp/capsem-v1.1.xxx`
- [ ] `gh release download v1.1.xxx --pattern manifest.json.minisig -D /tmp/capsem-v1.1.xxx`
- [ ] `minisign -Vm /tmp/capsem-v1.1.xxx/manifest.json -p config/manifest-sign.pub`
- [ ] `gh release download v1.1.xxx --pattern '*.pkg' -D /tmp/capsem-v1.1.xxx`
...
- [ ] `pkgutil --expand-full /tmp/capsem-v1.1.xxx/Capsem-*.pkg /tmp/capsem-v1.1.xxx/pkg-expanded`
- [ ] `gh release download v1.1.xxx --pattern '*.deb' -D /tmp/capsem-v1.1.xxx`
