...
Docker's legacy builder shared intermediate layer cache across `--platform` values, silently reusing arm64 layers for x86_64 builds. Fixed by requiring Docker BuildKit (buildx), which properly includes platform in cache keys. `just doctor` and `bootstrap.sh` now check for buildx and Colima Rosetta.
...
- Automatic incremental cache trimming when `cache/target/` exceeds 20 GB
...
- Standalone `build_system/scripts/doctor/kvm-diagnostic.py` for manual debugging
