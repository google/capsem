...
| Protocol | How |
|---|---|
| **HTTPS** (existing) | Guest -> iptables redirect -> capsem-net-proxy -> vsock:5002 -> host MITM proxy. Terminate TLS (per-domain cert from capsem CA), inspect HTTP, apply policy, forward to real upstream over TLS. |
...
capsem start --name dev
  ...
  Child (capsem-daemon):
    ...
    +-- macOS menu bar tray: NSStatusItem
    +-- CFRunLoop pumping (VZ requirement on macOS)
```

### Daemon auth on startup

1. Load `~/.capsem/authorized_keys` (or auto-populate from `~/.ssh/*.pub`)
2. Generate self-signed TLS server cert (ephemeral, in-memory)
3. Configure rustls `ServerConfig` with `SshAuthorizedKeysVerifier`
4. Bind Unix socket (local) + TLS listener (remote)
...
```
Returns `200 OK` when healthy, `503 Service Unavailable` when degraded. No auth required on `/health` (infra tools can't do mTLS).
...
For remote daemon: `HostName` points to daemon host.
...
5005  Reserved (was inotify watcher, replaced by VFS)
5006  SSH bridge (inbound only, MITM SSH, Phase 6)
```

Port 5005 freed up by VFS replacing the inotify watcher for filesystem telemetry. VFS provides file change visibility without a vsock channel.
