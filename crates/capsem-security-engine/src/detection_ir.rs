...
impl From<Severity> for crate::Severity {
...
impl From<Confidence> for crate::Confidence {
...
fn event_family_policy_root(event_family: EventFamily) -> Result<&'static str> {
    match event_family {
        ...
        EventFamily::Credential => Ok("credential"),
        EventFamily::Vm => Ok("vm"),
        EventFamily::Profile => Ok("profile"),
        EventFamily::Conversation => Ok("conversation"),
        EventFamily::Snapshot => Ok("snapshot"),
    }
}
...
fn is_supported_runtime_field(event_family: EventFamily, scope: &str, suffix: &str) -> bool {
    let normalized_suffix = normalize_indexed_field_path(suffix);
    let suffix = normalized_suffix.as_str();
    matches!(
        (event_family, scope, suffix),
        (
            EventFamily::Dns,
            "request",
            "qname" | "qtype" | "domain_class" | "transport"
        ) | (
            EventFamily::Http,
            "request",
            "method"
            ...
            EventFamily::Mcp,
            "request",
            "method" | "server_id" | "tool_name" | "server_name" | "arguments_status"
        ) | (
            EventFamily::Mcp,
            "response",
            "method" | "server_id" | "tool_name" | "is_error" | "result_status"
        ) | (
            EventFamily::Model,
            "request",
            "provider"
                ...
                | "tool_calls[*].name"
                | "tool_calls[*].origin"
                | "tool_calls[*].arguments_status"
                | "tool_calls[*].status"
                | "tool_calls[*].linked_mcp_call_id"
                | "tool_calls[*].parse_confidence",
        ) | (
            EventFamily::Model,
            "response",
            "provider"
                ...
                | "tool_results[*].linked_mcp_call_id"
                | "tool_results[*].content_kind"
                | "tool_results[*].content_preview"
                | "tool_results[*].content_json"
                | "tool_results[*].is_error"
                | "tool_results[*].result_status"
                | "tool_results[*].returned_to_model"
                | "tool_results[*].parse_confidence",
        ) | (
            EventFamily::File,
            "activity",
            ...
            EventFamily::Process,
            "activity",
            "operation" | "executable" | "command" | "command_class" | "cwd"
        ) | (
...
fn normalize_indexed_field_path(path: &str) -> String {
...
impl From<EngineEventFamily> for EventFamily {
    fn from(value: EngineEventFamily) -> Self {
        match value {
            ...
            EngineEventFamily::File => Self::File,
            EngineEventFamily::Snapshot => Self::Snapshot,
