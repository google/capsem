use super::*;
use crate::v2::format::{BLOCK_HEADER_BYTES, FILE_HEADER_BYTES, SEGMENT_HEADER_BYTES};
use crate::v2::tests::{archive, file_len};
use crate::v2::BodyLogReader;
...
fn open_refuses_a_version_one_archive_and_a_foreign_file() {
    let dir = tempfile::tempdir().unwrap();
    let path = archive(&dir);
    drop(crate::writer::BodyLogWriter::open(&path).unwrap());
