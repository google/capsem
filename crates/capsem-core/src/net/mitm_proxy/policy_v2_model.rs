...
pub async fn evaluate_model_request_policy(
    ...
    let request_outcome = match policy
...
async fn evaluate_model_tool_response_policy(
    ...
    for tool_result in &request_meta.tool_results {
        ...
        let matched = match policy.find_matching_rule(PolicyCallback::ModelToolResponse, &subject) {
...
pub async fn evaluate_model_response_policy(
    ...
    for (index, tool_call) in meta.tool_calls.iter().enumerate() {
        ...
        let matched = match policy.find_matching_rule(PolicyCallback::ModelToolCall, &subject) {
            Ok(Some(matched)) => matched,
            Ok(None) => continue,
            Err(error) => {
                return Some(ModelResponsePolicyOutcome::Deny(
