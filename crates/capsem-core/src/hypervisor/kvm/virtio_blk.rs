...
pub(super) struct VirtioBlockDevice {
    ...
    notify_tx: Option<mpsc::Sender<BlockWorkerCommand>>,
...
impl VirtioBlockDevice {
    ...
    pub fn with_interrupt(mut self, irq_fd: RawFd, interrupt_status: Arc<AtomicU32>) -> Self {
...
impl VirtioDevice for VirtioBlockDevice {
    ...
    fn activate(&mut self, mem: GuestMemoryRef, queues: &[QueueConfig]) {
        if let Some(q) = queues.first() {
            if q.size > 0 {
                ...
                if let (Some(irq_fd), Some(interrupt_status)) =
                    (self.irq_fd, self.interrupt_status.as_ref().cloned())
                {
                    match self.file.try_clone() {
                        Ok(file) => {
                            ...
                            let handle = std::thread::Builder::new()
                                ...
                                .expect("failed to spawn virtio-blk worker");
                        ...
                        Err(error) => {
                            tracing::warn!(
                                ...
                                %error,
                                "virtio-blk worker disabled because backing file clone failed"
                            );
    ...
    fn queue_notify(&mut self, queue_index: u32) {
        if let Some(tx) = self.notify_tx.as_ref() {
            let _ = tx.send(BlockWorkerCommand::Notify(queue_index));
            return;
        }
...
fn block_worker_loop(
    ...
    while let Ok(command) = rx.recv() {
        match command {
            BlockWorkerCommand::Notify(queue_index) => {
                if queue_index != 0 {
                    tracing::warn!(
                        event_name = "virtio.blk.queue_notify_ignored",
                        queue_index,
                ...
                tracing::trace!(
                    event_name = "virtio.blk.queue_drain",
                    processed,
                    "virtio-blk worker drained queue notification"
                );
            }
            BlockWorkerCommand::Drain(done) => {
                let processed = VirtioBlockDevice::process_queue(
                    &mut file,
                    read_only,
                    capacity_sectors,
...
mod tests {
    ...
    #[cfg(target_os = "linux")]
    #[test]
    fn block_worker_processes_notify_after_drain() {
        let mut data = vec![0u8; 512];
        for (i, byte) in data.iter_mut().enumerate().take(512) {
            *byte = (i % 251) as u8;
        }
        let path = temp_disk_with_data("worker-read.img", &data);
        let mut h = TestHarness::new_with_worker(&path, true);
        ...
        assert_eq!(h.read_status(status_offset), VIRTIO_BLK_S_OK);
        assert_eq!(
            h.interrupt_status.as_ref().unwrap().load(Ordering::SeqCst),
            1
        );
    }
