...
/// The forensic payload a security ledger stored for one event, read back out
/// of the session archive. It is a body rather than a column, so this is what
/// "the ledger kept it" means: the archive gives back the bytes the index
/// names, verified against the hash the writer recorded.
pub(super) async fn archived_payload_of(db_path: &std::path::Path, source_table: &str, event_id: &str) -> String {
    let body = capsem_logger::DbHandle::open_external_reader(db_path)
        .unwrap()
        .read_body(event_id, source_table, capsem_logger::BodyDirection::Payload)
        .await
        .unwrap()
...
async fn archived_payload(db_path: &std::path::Path, event_id: &str) -> String {
    archived_payload_of(db_path, "security_rule_events", event_id).await
}
...
"#,
    ...
    // The decision's forensic payload left its row for the archive, like the
    // rule matches' did, and it is the same evaluated event they carry: a
    // decision a reviewer cannot tie back to what was decided is not evidence.
    drop(conn);
    let event_id = emission.event_id.as_str();
    let decision_payload = archived_payload_of(&db_path, "security_decision_events", event_id).await;
    let rule_payload = archived_payload(&db_path, event_id).await;
    let decided: serde_json::Value = serde_json::from_str(&decision_payload).unwrap();
    assert_eq!(decided["event_type"], "http.request");
    assert_eq!(decided["http"]["host"], "api.example.com");
    assert_eq!(
        decision_payload, rule_payload,
        "the decision and the matches it came from describe one evaluated event"
    );
}
