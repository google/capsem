...
impl Registry {
    ...
    async fn start_with_auth(
        ...
        let app = Router::new()
            .fallback(
                move |State(blobs): State<Arc<BTreeMap<String, Vec<u8>>>>, request: Request<Body>| {
                    ...
                    async move {
                        ...
                        match blobs.get(request.uri().path()) {
                            Some(bytes) => {
                                (StatusCode::OK, [("content-type", "application/json")], bytes.clone()).into_response()
                            }
