//! Tests for `auth` (extracted from inline `mod tests`).
...
fn token_uniqueness_over_100_samples() {
    let tokens: std::collections::HashSet<String> = (0..100).map(|_| generate_token()).collect();
...
async fn all_non_root_paths_require_auth() {
    let app = test_app("tok");
    for path in ["/status", "/list", "/policy-hook/spec"] {
        let resp = app
            .clone()
