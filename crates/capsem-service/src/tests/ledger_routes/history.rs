...
/// Every arm of the page statement stops at `offset + limit` rows, walking
/// its table's timestamp index newest first, so a page costs its size rather
/// than the ledger's.
#[tokio::test]
async fn history_page_reads_through_the_timestamp_index() {
    let (_app, dir, _) = history_app().await;
    let conn = rusqlite::Connection::open(dir.path().join("sessions/history-page-vm/session.db")).unwrap();
    for layer in [
        api::HistoryLayerFilter::All,
        api::HistoryLayerFilter::Exec,
        api::HistoryLayerFilter::Audit,
    ] {
        for search in [false, true] {
            let sql = crate::ledger_routes::history::page_sql(layer, search);
            let arms = usize::from(layer != api::HistoryLayerFilter::Audit)
                + usize::from(layer != api::HistoryLayerFilter::Exec);
            assert_eq!(sql.matches("LIMIT").count(), arms + 1, "{sql}");
            let plan = query_plan(&conn, &sql, search);
            for (table, index) in [
                ("exec_events", "idx_exec_events_timestamp"),
                ("audit_events", "idx_audit_events_timestamp"),
            ] {
                if sql.contains(table) {
                    assert!(plan.contains(index), "{layer:?} search={search}: {plan}");
                }
            }
        }
    }
...
fn query_plan(conn: &rusqlite::Connection, sql: &str, search: bool) -> String {
    let needle = if search {
        rusqlite::types::Value::Text("needle".to_string())
    } else {
        rusqlite::types::Value::Null
    };
    let params = [needle, 20.into(), 10.into(), 10.into()];
