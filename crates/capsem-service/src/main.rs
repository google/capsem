        ...
        match remove(path) {
            ...
            Err(error)
                if error.kind() == std::io::ErrorKind::DirectoryNotEmpty
                    && attempt < SESSION_DELETE_MAX_ATTEMPTS =>
            {
                wait();
            }
...
impl ServiceState {
    ...
    fn delete_session_dir(&self, session_dir: &StdPath) -> Result<()> {
        ...
        remove_quiesced_session_dir(&canonical_session).with_context(|| {
            format!(
...
fn statvfs_bytes<BlockCount>(blocks: BlockCount, block_size: u64) -> u64
where
    BlockCount: Into<u64>,
{
...
fn refresh_reconcile_fields(
    ...
    if let Some(obj) = value.as_object_mut() {
        if obj.contains_key("downloading") {
            let active = reconcile.in_progress
                || state.asset_reconcile_inflight.load(Ordering::Acquire);
