...
fn vz_host_lock_is_mutually_exclusive() {
    ...
    let a = VzHostLock::acquire_test_path(&lock_path, VzHostLockMode::Exclusive, Duration::from_millis(50))
        .unwrap()
        .expect("first acquisition");
    let b = VzHostLock::acquire_test_path(&lock_path, VzHostLockMode::Exclusive, Duration::from_millis(50)).unwrap();
    assert!(
        b.is_none(),
    ...
    let c = VzHostLock::acquire_test_path(&lock_path, VzHostLockMode::Exclusive, Duration::from_millis(500))
        .unwrap()
        .expect("reacquire after drop");
    drop(c);
    drop(dir);
...
fn vz_host_lock_allows_shared_lifecycle_starts() {
    let (dir, lock_path) = isolated_vz_host_lock_path();
    let a = VzHostLock::acquire_test_path(&lock_path, VzHostLockMode::Shared, Duration::from_millis(50))
        .unwrap()
        .expect("first shared acquisition");
    let b = VzHostLock::acquire_test_path(&lock_path, VzHostLockMode::Shared, Duration::from_millis(50))
        .unwrap()
...
fn vz_host_lock_exclusive_blocks_shared() {
    ...
    let b = VzHostLock::acquire_test_path(&lock_path, VzHostLockMode::Shared, Duration::from_millis(50)).unwrap();
    assert!(
        b.is_none(),
