...
pub const SECURITY_EVENT_CEL_ROOTS: &[&str] = &[
    "http", "dns", "mcp", "model", "file", "process", "ip", "tcp", "udp", "network",
];
...
impl ProviderDiscovery {
    pub fn validate(&self, path: &str) -> Result<(), String> {
        ...
        if let Some(event_type) = self.event_type.as_deref() {
            if !crate::security_event_type_is_known(event_type) {
                return Err(format!(
                    "{path}.event_type: unknown runtime security event type '{event_type}'"
                ));
            }
        }
...
fn known_fields_for_root(root: &str) -> Vec<&'static str> {
    ...
    crate::SECURITY_EVENT_CEL_FIELDS
        .iter()
        .copied()
