...
fn replies_correlate_to_requests_by_id_and_broadcasts_answer_nothing() {
    let requests = [
        (ServiceToProcess::Exec { id: 7, command: "true".into() }, Some(7)),
        (ServiceToProcess::ReadFile { id: 8, path: "/x".into() }, Some(8)),
        (ServiceToProcess::SnapshotStatus { id: 9 }, Some(9)),
        (ServiceToProcess::McpRefreshTools { id: 10 }, Some(10)),
        (ServiceToProcess::LinkDetach { id: 11, network: "n".into(), generation: 1 }, Some(11)),
    ...
    let replies = [
        (
            ProcessToService::ExecResult { id: 7, stdout: vec![], stderr: vec![], exit_code: 0, truncated: false },
            Some(7),
        ),
        ...
        (ProcessToService::SuspendFailed { id: "vm".into(), error: "e".into() }, None),
        (ProcessToService::StateChanged { id: "vm".into(), state: "s".into(), trigger: "t".into() }, None),
