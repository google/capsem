...
All three run as part of `just test-full`. The production build is important -- Tailwind v4's Vite plugin can miss `client:only` components in the SSR module graph, so `@source` directives in `global.css` must explicitly include `.svelte` and `.ts` files.
...
**Generated mock data**: `mock-settings.generated.ts` is produced by `build_system/scripts/build/generate_schema.py` from `config/settings/ui-metadata.generated.json`. It runs as part of `just exec` and `just test-full` via the `_generate-settings` recipe. Never hand-edit this file.
...
1. Run `just _generate-settings` (or let `just exec`/`just test-full` do it)
2. Start `just dev ui`
