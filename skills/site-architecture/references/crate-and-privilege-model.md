...
- **`capsem-router`**: Seatbelt/seccomp-confined companion with two jobs from one binary. Per VM owner, a TCP relay that receives connected descriptor pairs over a private, bounded grant channel (published ports and private TCP between members). Per named network, `--switch`: a layer-2 switch that receives each member's frame link (a duplicate of the guest's VSOCK 5009 stream) from the service and forwards ethernet frames between members, pinning every frame's source MAC and address to its member, answering ARP itself, and dropping TCP, strangers and everything that is not IPv4 or ARP. No service control socket, ambient file access, listener acceptance, or virtualization entitlement in either job.
- **`capsem-network`**: the private link's frame codec (`u16` length + ethernet frame) and the switch verdict (`switch::classify`) as pure code: forward to exactly one member, answer, or drop for a named reason. The kernel inside each guest does ARP, IP and everything above; no host process parses past a frame's addresses.
