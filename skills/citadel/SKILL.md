...
`cache/target/gate-runs/DIGEST.md` is regenerated by `fast.digest` and again at
`RunLog.close`, computed from `cache/target/gate-runs/ledger.jsonl` -- one distilled
