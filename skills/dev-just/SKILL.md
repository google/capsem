...
| `just focus-test <group> [reuse\|clean]` | Rerun one existing owner: `assets`, `binaries`, `benchmark`, `functional`, `kingslanding`, `install`, `release-system`, or affected `rust`. `release-system` is source-only; `rust` derives changed crates and reverse dependents from Cargo manifests. |
