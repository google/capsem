...
`just test-clean`. Local development rebuilds every package and every profile.
...
- test composition: local `just test-clean` and both release lanes call
...
- VM assets: `just test-clean` owns `just _gate-assets`, which executes the same
...
- Linux package assembly: `just test-clean` executes `just _cross-compile arm64` and
...
- Linux platform branches: macOS-local `just test-clean` executes
...
- generated settings: `just test-clean` regenerates the tracked settings outputs and
...
Hold the release, reproduce the expensive rail through `just test-clean` locally, and
...
- **Linux `.deb` self-updates stop stale helpers before replacement.**
  ...
  registration/readiness so the old updater can activate its already-verified
...
- **Three files hold the binary version.** `Cargo.toml` (workspace),
  ...
  preparation keeps all three aligned before `just test-clean`; release workflows
...
- **Python coverage remains blocking at an exact 85% floor.** Ordinary CI runs
  its selected portable Python suite while full local `just test-clean` runs every
...
- **Do not execute artifact-dependent Python suites on a clean PR runner before
  ...
  `tests/capsem-*/` suite; the full `just test-clean` gate owns bootstrap/codesign
