...
boot. Local Apple Silicon `just test-clean` owns that VZ proof. Hosted macOS owns
...
zero means the segment passed, not that current source completed `just test-clean`.
