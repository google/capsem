...
- **No AppImage on arm64.** linuxdeploy has no arm64 build (the "continuous" aarch64 release is unstable and fails on CI runners). arm64 Linux (Chromebooks) ships `.deb` only. CI matrix passes `bundles: deb` for arm64, `bundles: deb,appimage` for x86_64. `just cross-compile` matches this logic. This cost v0.14.5 through v0.14.11 (7 failed releases) to discover.
...
- **`just cross-compile` is not a perfect CI replica.** It runs in a podman container on macOS, which has FUSE (via podman's Linux VM). CI runners may not have FUSE, so AppImage bundling that works locally can fail in CI. The recipe catches compile errors and most packaging issues, but environment differences (FUSE, linuxdeploy availability) can still slip through. Always verify the first CI run of a new Linux packaging change.
...
- **AppImage needs libfuse2 + `APPIMAGE_EXTRACT_AND_RUN=1`.** Ubuntu 24.04 runners ship FUSE3, but linuxdeploy and linuxdeploy-plugin-appimage are FUSE2 AppImages. Install `libfuse2` in the Tauri system deps step. Also set `APPIMAGE_EXTRACT_AND_RUN=1` on the `cargo tauri build` step as a belt-and-suspenders fallback. Without libfuse2, linuxdeploy silently fails with "failed to run linuxdeploy". This was the root cause of EVERY v0.14.x Linux build failure (14 consecutive failed releases from v0.12.1 through v0.14.12).
