...
- **`APPIMAGE_EXTRACT_AND_RUN=1` for AppImage builds.** GitHub CI runners lack FUSE, which linuxdeploy needs to self-extract. Set this env var on the `cargo tauri build` step. Without it, linuxdeploy silently fails with "failed to run linuxdeploy". This was the root cause of EVERY v0.14.x Linux build failure (14 consecutive failed releases from v0.12.1 through v0.14.12).
