...
`just test-clean`. Local `just test-clean` rebuilds both artifact families and runs the
...
`just test-clean` is the strict superset of portable CI work. CI workflows may run a
...
and `just _build-rootfs` primitives owned by `just test-clean` through
...
The accounting applies to host packages too: macOS-local `just test-clean` must build
